...
test_that("connect_to_model() stores model_path correctly", {
  cache <- getFromNamespace(".pkg_cache", "modelscloud")
  connect_to_model("resplab/qrisk3pexa")
  expect_equal(cache$model_path, "resplab/qrisk3pexa")
})

test_that("connect_to_model() stores access_key when supplied", {
  cache <- getFromNamespace(".pkg_cache", "modelscloud")
  connect_to_model("resplab/qrisk3pexa", access_key = "test-key-123")
  expect_equal(cache$access_key, "test-key-123")
})

test_that("connect_to_model() leaves access_key unchanged when NULL", {
  cache <- getFromNamespace(".pkg_cache", "modelscloud")
  connect_to_model("resplab/qrisk3pexa", access_key = "original-key")
  connect_to_model("resplab/qrisk3pexa")  # no access_key
...
test_that("connect_to_model() stores async flag", {
  cache <- getFromNamespace(".pkg_cache", "modelscloud")
  connect_to_model("resplab/qrisk3pexa", async = TRUE)
  expect_true(cache$async)

  connect_to_model("resplab/qrisk3pexa", async = FALSE)
...
test_that("connect_to_model() warns on bad server_url but does not store it", {
  ...
  expect_warning(
    connect_to_model("resplab/qrisk3pexa", server_url = "not-a-url"),
    "http"
  )
